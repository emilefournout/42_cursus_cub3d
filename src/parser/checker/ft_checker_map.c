...
/*   Updated: 2023/02/08 18:28:26 by josesanc         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static bool	ft_check_map_line(char *line)
{
	...
	while (line[index] != '\0')
	{
		if (line[index] != '1' && line[index] != '0'
			&& line[index] != 'S' && line[index] != 'N' && line[index] != 'E'
			&& line[index] != 'W' && ft_isspace(line[index]) == false
			&& ft_is_new_line(line[index]) == false)
			return (false);
		index++;
	}
	return (true);
