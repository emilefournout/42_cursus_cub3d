...
/*   Updated: 2023/02/09 17:30:39 by josesanc         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static bool	ft_check_texture_bonus(t_list *extracted_input,
									t_extracted_input *element)
{
	element->door = ft_c_txtr("DO", extracted_input);
	if (element->door == (void *)1 || element->door == (void *)2
		|| ft_check_extracted_element(element->door) == false)
		return (ft_print_error(BAD_TXTR), false);
	element->sprite = ft_c_txtr("SP", extracted_input);
	if (element->sprite == (void *)1 || element->sprite == (void *)2
		|| ft_check_extracted_element(element->sprite) == false)
		return (ft_print_error(BAD_TXTR), false);
	return (true);
}
...
bool	ft_checker(t_list *extracted_input, t_extracted_input *element)
{
	if (ft_check_texture_color(extracted_input, element) == false)
		return (false);
	if (ft_check_texture_bonus(extracted_input, element) == false)
		return (false);
